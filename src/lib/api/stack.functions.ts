...
async function isAdmin(userId: string) {
  ...
  // Let's check profile role if user is in profiles, otherwise check if they are user ID
  // In our auth-supabase, profile.role || 'User' is used.
  // For the sake of simplicity in SQLite users, we check if name is admin or if they have email admin@airank.com
  return userResult[0].email === "admin@airank.com" || userResult[0].name.toLowerCase().includes("admin");
}
...
export const createStackFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!checkRateLimit(`create-stack:${authData.user.id}`, 20, 1000 * 60 * 15)) {
...
export const addCommentFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const ip = getRequestHeader("x-forwarded-for") || "unknown";
    if (!checkRateLimit(`comment:${authData.user.id}`, 10, 1000 * 60 * 5)) {
...
export const getRecommendationsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    // Beginner: prefer free or highly rated/easy-to-use tools. Let's make sure we have 4 strong recommendations.
    ...
    // Limit to 4 tools in the recommended stack
    const finalStackTools = finalSelection.slice(0, 4);
