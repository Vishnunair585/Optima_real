...
import { Menu, X, Sun, Moon, LogOut, User as UserIcon, Settings as SettingsIcon, CreditCard as CreditCardIcon, FolderClosed as FolderClosedIcon, SplitSquareHorizontal } from "lucide-react";
    ...
    <header className={`sticky top-0 z-50 transition-all ${scrolled || isOpen ? "border-b border-border glass-strong" : ""}`}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to="/" className="flex shrink-0 items-center gap-2.5">
          ...
          <span className="hidden rounded-full border border-border bg-muted/40 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground sm:inline">beta</span>
        </Link>
