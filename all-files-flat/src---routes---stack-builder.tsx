...
function StackBuilder() {
  ...
  useEffect(() => {
    async function loadStack() {
      if (editId) {
        try {
          const stack = await getStackByIdOrSlugFn({ data: { id: editId } });
          if (stack) {
            setName(stack.name);
  ...
  const handleWizardSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const rec = await getRecommendationsFn({ data: {
      ...
      if (!rec?.tools?.length) {
        toast.error("No matching tools found. Try selecting different categories.");
        return;
      }
  ...
  const handleSaveStack = async () => {
    ...
    const stackDescription = description.trim() || `A custom ${category} workflow built with ${selectedTools.length} AI tools.`;
    const stackGoal = goal.trim() || `Optimize ${category.toLowerCase()} workflows`;

    setIsSaving(true);
    try {
      if (editId) {
        const updateRes = await updateStackFn({ data: {
          id: editId,
          name: name.trim(),
        ...
        if (updateRes && typeof updateRes === 'object' && 'success' in updateRes && updateRes.success) {
          ...
          toast.error("Failed to update stack. Please try again.");
        }
      } else {
        const res = await createStackFn({ data: {
          name: name.trim(),
          description: stackDescription,
          goal: stackGoal,
          category,
        ...
        if (res?.paywall || res?.success === false) {
          toast.error(
            res.reason ??
              `You've reached your saved stack limit (${res.current ?? "?"}/${res.limit ?? "?"}). Upgrade to Pro for unlimited stacks.`,
            {
              action: {
                label: "Upgrade",
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-border/40 pb-6 mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isSaving}
            className="inline-flex h-10 items-center gap-2 rounded-full bg-gradient-brand px-5 text-sm font-semibold text-brand-foreground shadow-glow disabled:opacity-60"
          >
            <Save className="h-4 w-4" /> {isSaving ? "Saving..." : "Save Stack"}
          </button>
        </div>
      </header>
              ...
              <form onSubmit={handleWizardSubmit} className="space-y-4">
                ...
                <div className="flex justify-end pt-4 border-t border-border/40">
                  <button
                    ...
                    className="inline-flex h-10 items-center justify-center rounded-full bg-gradient-brand px-6 text-sm font-semibold text-brand-foreground shadow-glow disabled:opacity-60"
                  >
                    {isGeneratingRec ? (
