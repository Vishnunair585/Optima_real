...
import { recordFeatureUsage } from "./billing.functions";
...
export const saveComparisonFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!await rateLimit(`save-compare:${authData.user.id}`, 20, 1000 * 60 * 15)) {
    ...
    const gate = await recordFeatureUsage(authData.user.id, "comparison", { tools: data.tools });
    if (!gate.allowed) {
      return {
        success: false,
        paywall: true,
